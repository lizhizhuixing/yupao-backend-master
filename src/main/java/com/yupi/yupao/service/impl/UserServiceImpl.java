...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
     ...
     */
    @Override
    public List<User> usersSearchByTags(List<String>tagListName){
        ...
        //第一种方式使用sql语句进行查询
        //使用字符串拼接的方式进行查询
//        for (String tagName : tagListName) {
...
//        return   usersList.stream().map(this::getSafetyUser).collect(Collectors.toList());
    //第二种方式使用内存查询
        ...
        return usersList.stream().filter(user -> {
            ...
            Set<String> tempTagsName = gson.fromJson(tagStr, new TypeToken<Set<String>>() {
            }.getType());
    ...
    public boolean isAdmin(HttpServletRequest httpServletRequest) {
        ...
        if (user == null && userRole != UserConstant.ADMIN_ROLE) {

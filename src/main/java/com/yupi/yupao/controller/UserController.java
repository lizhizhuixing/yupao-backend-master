...
public class UserController {
    ...
    public BaseResponse<List<User>> searchByTags(@RequestParam(required = false) List<String> tagListName){
        if (tagListName == null){
            throw  new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        List<User> users = userService.usersSearchByTags(tagListName);
    ...
    public boolean isAdmin(HttpServletRequest httpServletRequest) {
        return userService.isAdmin(httpServletRequest);
    ...
    public BaseResponse<Page<User>> recommendUsers(long pageSize, long pageNum, HttpServletRequest request){
        ...
        String redisKey = String.format("huoji:user:recommend:%s",logininUser.getId());
        ValueOperations valueOperations = redisTemplate.opsForValue();
        //一种情况，缓存数据中已经存在缓存了，可以直接读取
        Page<User> userPage = (Page<User>) valueOperations.get(redisKey);
        if (userPage != null){
            return ResultUtils.success(userPage);
        }
        //一种情况，缓存数据中为空，需要先读取数据库中的缓存，并且把数据库中的缓存写入到redis中
        QueryWrapper<User> queryWrapper = new QueryWrapper<>();
